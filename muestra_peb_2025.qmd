...
Para este fin usaremos un método de calibración lo bastante general como el GREG [@sarndalCalibrationApproachSurvey] para así poder incluir tanto variables categóricas como continuas y que permita la inclusión no solo los valores de tendencia central sino también la varianza o distribución de las variables continuas. Esto, como veremos a continuación, genera diferentes problemas a la hora de adaptar/transformar los insumos necesarios para la calibración.
...
Variables como sector, ambito, jornada completa y región son categóricas y tienen origen en el marco muestral, ya que son variables de los establecimientos. De cada una de estas se van a extraer los respectivos porcentajes de cada categoría (p.e. sector_privado, ambito_rural_agrupado, etc.) ponderadas por la matrícula. El supuesto, creemos que relativamente inocente en términos agregados, es que para esto no es necesario utilizar la matrícula específica de tercer año lo que hubiera requerdio trabajar con la base de secciones. El punto negativo de esta base es que hay establecimientos del marco muestral que no están presentes en la base de secciones por lo que hubiera sido necesario estimar mediante algún modelo el tamaño de las secciones faltantes.

En el caso de las variables continuas como porcentaje de AUH (marco muestral) y las notas del IRA (censo PEB) la situación es diferente. En el primer caso, se realiza una media ponderada (por matrícula) y luego se multiplica por la cantidad absoluta de estudiantes en 3.er grado. En el caso de las notas IRA se utiliza la media (no ponderada) y luego se multiplica por la cantidad absoluta de estudiantes en 3.er grado. La cantidad absoluta de estudiantes en tercer grado puede salir tanto de las bases de secciones o de las pruebas PEB. Previsiblemnete, la primera estrategia permita un mayor acercamiento a la población objetivo y la segunda, en cambio, permite una mayor coherencia con el censo.
...
print("--- Totales de Control: Variables Categóricas ---")
print(df_tot_calib_marco_muestra_discretos)
...
print("--- Total de Control: Variable Continua ---")
print(tot_calib_marco_muestra_continuos)

insumo_calibracion_3 = insumo_calibracion_3 |>
left_join(calib_marco_muestra_continuos, by = "clave")
```

Ahora vamos a preparar los totales que provienen del censo de las PEB. Estos tienen que ver con variables continuas como las IRA de matemática y las IRA de prácticas del lenguaje.
...
df_tot_ira_mat_peb_3 = tibble(
    ...
    total = 68.7 * n_poblacion_objetivo_3$matricula_3)

df_tot_ira_pl_peb_3 = tibble(
    nombre_parametro = "ira_pl",
    total = 74.5 * n_poblacion_objetivo_3$matricula_3)
...
# Los totlaes estan en los objetos "tot_calib_marco_muestra_discretos", "tot_calib_marco_muestra_continuos" y "tot_peb_3".
...
df_totales_unificado_reducido = df_totales_unificado |>
  filter(!nombre_parametro %in% c("sectorEstatal", "region01", "jornada_completaNO", "ambitoRural Agrupado"))

mi_formula = ~ ambito + jornada_completa + region + sector + auh_pct + ira_mat + ira_pl

# 2. Extraer la matriz de diseño que R construye para su muestra
matriz_muestra = model.matrix(mi_formula, data = insumo_calibracion_3)
nombres_oficiales = colnames(matriz_muestra)

# 3. VERIFICACIÓN EN CONSOLA (Diagnóstico)
# Compare visualmente los nombres oficiales contra los nombres de su vector:
print("Nombres que R espera:")
print(nombres_oficiales)

print("Nombres que tiene su vector actualmente:")
print(names(totales_calibracion))
...
calibrate(design = insumo_calibracion_3_sv,
          formula = ~ ambito + jornada_completa + region + sector + auh_pct + ira_mat + ira_pl,
          population = totales_calibracion,
        #  stage = 1, Esto era si solo se le ponia información de los estab
          calfun = "linear",
          verbose = FALSE,
       #  variance = 
         )
...
mutate(ira_mat_count = ira_mat * n_poblacion_6$n_estudiantes_6,
       ira_pl_count = ira_pl * n_poblacion_6$n_estudiantes_6)
```
