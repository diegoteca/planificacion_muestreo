...
tbl_weight_design_test_2 = insumo_calibracion_3 |> 
  summarise(
    n_muestra = n(),
...
En la @fig-no_respuesta_3_auh, se realiza la misma operación pero ahora en función del porcentaje de AUH del establecimiento.
...
Luego de algunos ejercicios exploratorios se vió que eran pertienente incorporar algunos efectos de interacción entre las variables principales anteriores. Más precisamente, teniendo en cuenta la magnitud de los errores y la cantidad de casos de cada categoría, se prefirió incorporar la interacción entre:
...
```{r}
#| label: no_respuesta_propensity_score
#| echo: true
#| code-fold: true
df_no_respuesta = marco_muestra |>
filter(muestra_2025 == 1) |>
...
```{r}
#| label: tbl-comparacion_pasos_nr
...
prob_quantiles = quantile(
  insumo_calibracion_3$prob_respuesta,
  probs = probs_diag, na.rm = TRUE
)

df_diagnostico_inv_nr = tibble(
  Percentil = c("Mínimo (P0)", "P5", "P25", "P50 (Mediana)",
                "P75", "P95", "P99", "Máximo (P100)"),
  prob_respuesta = prob_quantiles,
  inv_no_respuesta = 1 / prob_quantiles
)

# Genero la tabla gt
df_diagnostico_inv_nr |>
  gt() |>
  tab_header(
...
```{r}
#| label: tbl-cruces_nr_interacciones
...
calcular_cruce = function(data, var1, var2) {
  data |>
    mutate(
...
cruces_nr = bind_rows(
  calcular_cruce(df_no_respuesta, "Region", "sector"),
  calcular_cruce(df_no_respuesta, "Region", "ambito"),
  calcular_cruce(df_no_respuesta, "sector", "jornada_completa"),
  calcular_cruce(df_no_respuesta, "ambito", "jornada_completa") 
)
...
Finalmente, en la @tbl-impacto_nr_categoricas comparo las proporciones expandidas de las variables categóricas de calibración bajo los dos esquemas de ponderación. Esto permite identificar qué categorías se ven más afectadas por el ajuste de no respuesta y en qué dirección.

```{r}
#| label: tbl-impacto_nr_categoricas
...
temp_diag_nr = insumo_calibracion_3 |>
...
tbl_diag_orig = temp_diag_original |>
  tbl_summary(
...
tbl_impacto_nr = tbl_merge(
  tbls = list(tbl_diag_wd, tbl_diag_wnr, tbl_diag_orig),
...
Finalmente, construidos los ponderadores base o de diseño (`weight_design`), sobre los cuales se realizaron las correcciones por no-respuesta (`weight_no_respuesta`), ahora pasamos al proceso de calibración propiamente dicho. Para esto usaremos diferentes fuentes auxiliares. Algunas tendrán origen en el propio marco muestral y otras tendrán origen en los datos agregados que informaron los establecimientos sobre los resultados de las PEB. Cabe destacar que ambas fuentes pueden no coincidir dado que los resultados de las PEB no fueron informados por todas los establecimientos. El punto imporante de incluir información censal de las PEB es, aparte de reducir la discrepancia en las publicaciones entre los resultados censales y los muestrales, es que informa sobre una serie de variables que eran consideradas claves en los objetivos de las PEB.
...
Variables como sector, ambito, jornada completa y región son categóricas y tienen origen en el marco muestral, ya que son variables de los establecimientos. De cada una de estas se van a extraer los respectivos porcentajes de cada categoría (p.e. sector_privado, ambito_rural_agrupado, etc.) ponderadas por la matrícula. El supuesto, creemos que relativamente inocente en términos agregados, es que para esto no es necesario utilizar la matrícula específica de tercer año lo que hubiera requerido trabajar con la base de secciones. El punto negativo de esta base es que hay establecimientos del marco muestral que no están presentes en la base de secciones por lo que hubiera sido necesario estimar mediante algún modelo el tamaño de las secciones faltantes. La solución intermedia fue la siguiente. Extraer de la base de secciones el ratio entre el total de la matrícula de la primaria y la matrícula de 3 y 6 año respectivamente. Dado que esta base tiene algunos pequeños faltantes lo importante es el ratio y no el total. Este último se extrae de la propia base de establecimientos y luego se le aplica el respectivo ratio.

En el caso de las variables continuas como porcentaje de AUH (marco muestral) y las notas del IRA (censo PEB) la situación es diferente. En el primer caso, se realiza una media ponderada (por matrícula) y luego se multiplica por la cantidad absoluta de estudiantes en 3.er io 6.º grado. En el caso de las notas IRA se utiliza la media (no ponderada) y luego se multiplica por la cantidad absoluta de estudiantes en 3.er o 6.º grado. La cantidad absoluta de estudiantes en cada grado puede salir tanto de las bases de secciones o de las pruebas PEB. Previsiblemente, la primera estrategia permita un mayor acercamiento a la población objetivo y la segunda, en cambio, permite una mayor coherencia con el censo. El mayor problema de intentar calibrar valores con el censo PEB es que se está trabajando con variables que suelen tener una masa no marginal de valores faltantes.

En general, intentar calibrar valores faltantes (como algo diferentre a casos faltantes) suele ser algo problemático. Concretamente, la mayoria de los algoritmos disponibles en la bibliografía solo permiten calibrar sobre casos que tengan valores completos.

```{r}
#| label: totales_discretos_calibracion_marco_muestra
#| echo: true
#| code-fold: true
calib_marco_muestra_discretos = marco_muestra |>
select(clave, matricula_3, Region, ambito, sector, jornada_completa) |>
rename(region = Region)

df_tot_calib_marco_muestra_discretos = calib_marco_muestra_discretos |>
...
insumo_calibracion_3 = insumo_calibracion_3 |>
left_join(calib_marco_muestra_discretos, by = "clave")

```

```{r}
#| label: totales_continuos_calibracion_marco_muestra
#| echo: true
#| code-fold: true
...
calib_marco_muestra_continuos = marco_muestra |>
select(clave, auh_pct, matricula_3) |>
mutate(auh_pct = auh_pct / 100)

df_tot_calib_marco_muestra_continuos = calib_marco_muestra_continuos |>
...
calib_marco_muestra_continuos = calib_marco_muestra_continuos |>
...
Ahora vamos a preparar los totales que provienen del censo de las PEB. Estos tienen que ver con variables continuas como el IRA de matemática y el IRA de prácticas del lenguaje. Es importate destacar que por razones de no respuesta, en la muestra, existen una fracción no marginal de estudiantes que han completado parte de las preguntas de matemática y no las de prácticas del lenguaje (o viceversa). Esto genera un problema a la hora de la construcción de los calibradores porque, en general, los algoritmos utilizados no permiten la introducción de casos sin respuesta. Esto, previsiblemente, no sucedía con los totales anteriores porque los mismos se imputaban a los estudiantes por su pertenencia a tal o cual establecimiento.

Esto mismo, aunque a una menor escala por tratarse de una base agregada a nivel de establecimientos,, también sucede en la base censal de las PEB. La base censal presenta sus particulares problemas de subregistro precisamente porque algunos establecimientos no han informado valores o lo han informado por matemática y no por PL o viceversa. Esto no es un problema menor porque la construcción de los calibradores requiere el uso de totales aparte de las medias o proporciones. En este sentido, aun cuando se asuma que, por ejemplo, las medias de las IRA censales no se encuentran sesgadas (p.e. la no respuesta de los establecimeintos es azaosa) queda pendiente o abierta el criterio de a cuantos casos representa esas medias. En principio, acá vamos a asumir que el censo no está sesgado, pero si que posee un subregistro con respecto a la población objetivo. De este modo, el criterio de "a cuantos" representa cada media de las IRA, se responderá de modo análogo a resto de las variables usadas en la calibración que no tienen origen en el censo de las PEB.

Esto se solucionará de un modo específico dada la particularidad que esta muestra es parte complementaria del respectivo censo de las PEB. Esto permite agregar a la base de la muestra una variable que informe sobre la nota agregada de cada materia y cada establecimiento. Esat variable, salvo casos marginales, no debería tener casos sin respuesta por lo que se puede utilizar como variable a calibrar con los totales poblacionales del censo.

```{r}
#| label: totales_continuos_calibracion_censo_peb
...
Sin embargo, como se observa en la @tbl-tot_peb_3_xestab, existe una proporción pequeña de establecimientos que poseen valores faltantes aun cuando estos se agrupen por establecimiento. Para poder calibrar por las notas de las IRA vamos a estimar, a nivel de los establecimientos.
...
```{r}
#| label: tbl-comparacion_calib_poblacion
...
chequeo_pesos_resumen <- insumo_calibracion_3 |>
  summarise(
...
chequeo_ratio <- insumo_calibracion_3 |>
  summarise(
...
df_calidad <- tibble(
  ...
  Valor = c(
    chequeo_pesos_resumen$peso_min,
    chequeo_pesos_resumen$peso_max,
    chequeo_pesos_resumen$casos_negativos,
    chequeo_pesos_resumen$casos_menores_uno,
    chequeo_pesos_resumen$suma_pesos_diseno,
    chequeo_pesos_resumen$suma_pesos_no_resp,
    chequeo_pesos_resumen$suma_pesos_calibrados,
    chequeo_pesos_resumen$n_muestra,
    chequeo_ratio$ratio_min,
    chequeo_ratio$ratio_p25,
    chequeo_ratio$ratio_med,
    chequeo_ratio$ratio_p75,
    chequeo_ratio$ratio_max,
    chequeo_ratio$ratio_sd
    #, deff_kish
  )
)

# 5. Generación de la tabla gt estética
tbl_calidad_gt <- df_calidad |>
...
Por último, vamos a realizar algunos cruces entre los valores de los IRA y sector.

```{r}


# 1. Obtenemos los conteos/totales ponderados de la población cruzando sector e ira_mat
df_sector_ira = as_survey(muestra_peb_3_cal) |>
group_by(sector) |>
summarize(ira_mat = survey_mean(ira_mat),
          ira_pl = survey_mean(ira_pl))

df_ira = as_survey(muestra_peb_3_cal) |>
summarize(ira_mat = survey_mean(ira_mat),
          ira_pl = survey_mean(ira_pl))

df_auh = as_survey(muestra_peb_3_cal) |>
group_by(tercil_AUH) |>
summarize(ira_mat = survey_mean(ira_mat),
          ira_pl = survey_mean(ira_pl))

df_auh_sector_ira = as_survey(muestra_peb_3_cal) |>
group_by(sector, tercil_AUH) |>
summarize(ira_mat = survey_mean(ira_mat),
          ira_pl = survey_mean(ira_pl))
```

```{r}
# Ahora hacemos el cruce según terciles de AUH
```
...
ratio_primaria_sexto = n_poblacion_primaria_base_secciones$matricula / 
                          n_poblacion_3_base_secciones$matricula

# Esto estima la matricula_3 de cada establecimiento
marco_muestra = marco_muestra |>
mutate(matricula_6 = matricula_inicial_2025 / ratio_primaria_sexto)

n_poblacion_objetivo_6 = marco_muestra |>
summarise(matricula_6 = sum(matricula_inicial_2025)/ratio_primaria_sexto)
