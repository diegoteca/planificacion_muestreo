...
En cualquier caso, siempre es importante distinguir cuáles son los objetivos, cuáles son las restricciones y cuáles son los márgenes de maniobra para obetener una solución relativamente óptima al problema [@valliant2018, cap. 5].
...
La realización de diseño estratificado no proporcional tiene el riesgo que su resultado es muy dependiente de cual sea la variable objetivo [@tillé2020, pág. 75]. En contextos en donde exista más de una variable de interés (lo usual en muchas investigaciones sociales) y/o en casos en donde se presuma que determinada fuente primaria luego será utilizada como fuente secundaria por otros investigadores con objetivos que todavía se desconocen, se debe analizar los pros y contras de esta elección. Por ejemplo, algunos análisis estadísticos complejos (todavía) no permiten incorporar los valores de los ponderadores/expansores en sus cálculos.

Por otro lado, cuanto más estratos se construyan, más preciso será el estimador. Sin embargo, cabe advertir que un caso por cada estrato no es un buen diseño muestral [@tillé2020, pág. 77]. La razónde esto es que, usualmente, el estimador de la varianza se convierte cada vez más inestable como el número de estatos se acrecienta por la pérdida de grados de libertad derivada de la estimación de la media de los estratos.
...
fig_mapa_muestra_cubo_poblacion_distritos = tm_shape(datos_facelas) +
  tm_polygons(
   fill = "matricula_pct",
   fill.scale = tm_scale_intervals(style = "quantile", n = 5),       # Los quiebres se calcularán sobre los valores de porcentaje
   fill.legend = tm_legend(title = "% del Total"),
   fill.free = FALSE,
    popup.vars = c("Matricula" = "Matricula_Total", 
                   "Matrícula %" = "matricula_pct") # Utilidad interactiva
  ) +
  tm_facets(by = "Tipo_Analisis", ncol = 2) +
  tm_layout(
    legend.outside = TRUE,
    panel.label.size = 1.2
  )

fig_mapa_muestra_cubo_poblacion_distritos
```
...
# 2. Mapa coroplético basado en el porcentaje
fig_mapa_muestra_cubo_poblacion_regiones_a = tm_shape(datos_facelas) +
  tm_polygons(
    fill = "Porcentaje",
    fill.scale = tm_scale_intervals(style = "quantile", n = 5),       # Los quiebres se calcularán sobre los valores de porcentaje
    fill.legend = tm_legend(title = "% del Total"),
    fill.free = FALSE,
    popup.vars = c("Absoluto" = "Matricula_Total", "Porcentaje" = "Porcentaje") # Utilidad interactiva
  ) +
  tm_facets(by = "Tipo_Analisis", ncol = 2) +
  tm_layout(
    legend.outside = TRUE,
...
En base a los datos de la figura @fig-densidad_ivse_poblacion_muestra, ahora vamos a testear estadísticamente si la diferencia entre ambas distribuciones es significativa.
