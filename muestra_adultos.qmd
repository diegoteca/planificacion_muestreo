...
![![](images/paste-2.png)](images/paste-1.png){width="745"}
...
df_universo = read_xlsx(here("Inputs", "muestra_adultos", "Muestra Encuesta DEJAyAM 2026.xlsx")) |>
clean_names() |>
...
```{r}
#| label: tbl-explo_marco_discretas_establecimientos
...
tbl_explo_marco_discretas_establecimientos = df_sampling_frame |>
select(region, region_agrupada, ambito, area, nivel, tipo_est_adulto, matricula, secciones, estudiante_seccion, ivse) |>
tbl_summary(
...
```{r}
#| label: fig-explo_marco_continuas_establecimientos
#| fig-cap: Análisis exploratorio variables continuas - Nivel establecimientos
# graficar con ggplot y ggplotly
fig_explo_marco_continuas_establecimientos = df_sampling_frame |>
...
```{r}
#| label: tbl-explo_marco_discretas_estudiantes
#| tbl-cap: Análisis exploratorio variables discretas - Nivel estudiantes
# Las tablas con gtsummary y los gráficos con ggplot y luego ggploly para hacerlo interactivo
tbl_explo_marco_discretas_estudiantes = df_estudiantes |>
select(region, region_agrupada, ambito, area, nivel, tipo_est_adulto, matricula, secciones, estudiante_seccion, ivse) |>
tbl_summary(
           sort = all_categorical() ~ "frequency",
           label = list(region = "Región",
...
```{r}
#| label: fig-explo_marco_continuas_estudiantes
#| fig-cap: Análisis exploratorio variables continuas - Nivel establecimientos
# graficar con ggplot y ggplotly
fig_explo_marco_continuas_estudiantes = df_estudiantes |>
select(matricula, secciones, estudiante_seccion, ivse) |>
pivot_longer(cols = everything(), names_to = "variable", values_to = "valor") |>
ggplot(aes(x = valor)) +
geom_histogram(bins = 30, fill = "#69b3a2", color = "#e9ecef", alpha = 0.9) +
...
tbl_comparacion_establecimientos_estudiantes = tbl_merge( 
 tbls = list(tbl_explo_marco_discretas_establecimientos, 
             tbl_explo_marco_discretas_estudiantes),
...
fig_comparacion_establecimientos_estudiantes = fig_explo_marco_continuas_establecimientos / fig_explo_marco_continuas_estudiantes
...
```{r}
#| label: tbl-dominios_estimacion_establecimientos
...
```{r}
#| label: tbl-dominios_estimacion_estudiantes
...
#| tbl-cap: Compracion dominios de estimación. Nivel establecimientos y nivel estudiantes.
...
Para ser consistente tanto con el pedido de precisión original (Nivel de confianza del $90\%$, $p=50\%$ y márgenes de error de $3\%$, $4\%$ y $5\%$) como con el supuesto de la superioridad de los CV, podemos establecer la siguiente equivalencia teórica. Si recordamos que el margen de error absoluto es:

ESTA FÓRMULA ESTÁ MAL

$d = z_{1-\alpha/2} \cdot SE(\hat{p})$
...
$$CV(\hat{p}) = \frac{SE(\hat{p})}{p} = \frac{d}{p \cdot z_{1-\alpha/2}} = \frac{d}{0,5 \cdot 1,645} \approx \frac{d}{0,8225}$$ {#eq-margen-cv}
...
convertir_mde_a_cv = function(mde, 
                              z = z_valor, 
                              p = p_varianza_max) 
{
  se_objetivo = mde / z # Estándar error
...
cv_o_te_area = convertir_mde_a_cv(mde = 0.03)    # Para EPA y CENS (3% MdE)
cv_o_te_region_agrupada = convertir_mde_a_cv(mde = 0.04)  # Para FINES individual (5% MdE)
cv_o_te_region = convertir_mde_a_cv(mde = 0.05)   # Para FINES grupo de regiones (4% MdE)


# Presentamos los resultados formateados
resultados_precision = tibble(
  Objetivo = c("TE/Área", "TE/Región", "TE/Región agrup."),
  MdE_Original = c("3.0%", "4.0%", "5.0%"),
...
```{r}
#| label: tbl-traduccion_mde_cv
#| tbl-cap: Traducción de MdE a CV
tbl_traduccion_mde_cv = resultados_precision |> 
  select(Objetivo, MdE_Original, CV_Objetivo_Pct) |> 
  gt() |> 
  tab_header(
  ...
  cols_label(
    ...
    MdE_Original = "Margen de Error (MdE)",
    CV_Objetivo_Pct = "CV Objetivo"
  )
tbl_traduccion_mde_cv
```
...
estimar_tamano_estrato = function(mde_objetivo, 
                                  deff = deff_real, 
                                  p = p_estimada) {
  # Calculamos el CV correspondiente a ese MdE
  cv_objetivo = (mde_objetivo / confianza_z) / p
...
muestra_te_area = estimar_tamano_estrato(mde_objetivo = 0.03)   # 3% MdE
muestra_fines_grupo = estimar_tamano_estrato(mde_objetivo = 0.04) # 4% MdE
muestra_fines_individual = estimar_tamano_estrato(mde_objetivo = 0.05) # 5% MdE
...
Una vez calculados los tamaños muestrales teóricos bajo el supuesto de universos infinitos o muy grandes, el paso metodológico fundamental es confrontar estos requerimientos con el tamaño real de nuestro marco muestral (@sec-marco_muestral_adultos).
...
1\. **La Corrección por Población Finita (FPC):** Cuando el tamaño de muestra de sedes calculado ($n_h$) representa una fracción significativa del total de sedes disponibles en el universo de ese estrato ($N_h$, por ejemplo, más del $5\%$), la varianza del estimador disminuye porque conocemos a una porción muy grande de la población. Para evitar un sobremuestreo innecesario y costoso, corregimos el tamaño muestral aplicando el factor de población finita: $$n_{\text{corregido}, h} = \frac{n_h}{1 + \frac{n_h}{N_h}}$$ 2. **Estratos pequeños e inferencia regional:** En ofertas como FINES en regiones del interior muy pequeñas, el requerimiento teórico de precisión regional (5% MdE, que exige unas 76 sedes por región) puede superar la cantidad física de sedes disponibles en el universo de esa región. En esos casos, el estrato se vuelve automáticamente **censal** (se seleccionan todas las sedes del estrato, asignándoles probabilidad de inclusión $\pi_i = 1$).
...
En un muestreo PPS, la probabilidad de selección de la sede $i$ en el estrato $h$ se define como: $$\pi_{ih} = n_{\text{corregida}, h} \cdot \frac{\text{Matrícula}_{ih}}{\text{Matrícula Total}_h}$$
...
calcular_pi_pps = function(matricula_vector, n_sedes_muestra) {
...
df_marco_seleccion = df_marco_seleccion |> 
  left_join(resumen_asignacion |> select(estrato_diseno, sedes_corregidas_n), by = "estrato_diseno") |> 
  group_by(estrato_diseno) |> 
  mutate(
    pi_pps = calcular_pi_pps(matricula, first(sedes_corregidas_n))
  ) |> 
  ungroup()
