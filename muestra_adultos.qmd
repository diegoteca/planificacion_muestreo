...
n = 800 # Esto es 500 menos que la primera versión
...
df_insumo_cubo = df_insumo_cubo |>
  left_join(regional_shares |> select(region_agrupada, nudge_power), by = "region_agrupada") |>
  mutate(
    # Calculo el factor de ajuste según el tipo de establecimiento para penalizar los más grandes
    factor_ajuste_CENS_EPA = case_when(
      tipo_est_adulto == "CENS" ~ reductor_cens,
      tipo_est_adulto == "EPA" ~ reductor_epa,
      .default = 1.0
    ),
    # Si es censal va al 100%, si no, se ajusta por tamaño de la matrícula (pps), el "nudge" regional y el factor por tipo
    weight = if_else(censal == 1, 
                     99999999, 
                     matricula * coalesce(nudge_power, 1) * factor_ajuste_CENS_EPA)
  )
...
El primer chequeo que se realizará tendra que ver con los porcentajes de establecimientos seleccionados por dominio de estimación. La función básica de esto es observar el efecto del "*power allocation*" que permitió que aumentar la participación porcentual de los dominios más pequeños y reducir la participación porcentual de los dominios más grandes. Para visualizar este efecto, en la siguiente tabla se muestran los resultados porcentuales con y sin ponderar. La idea es que, sin ponderar, el porcentaje de los dominios pequeños sea mayor por efecto de una sobrerepreentación y que con la ponderación, el porcentaje de los dominios pequeños se acerque al porcentaje real que tienen en la población.
...
#| fig-cap: Distribución de densidad de evse según dominios. Población y muestra adultos.
#| fig-height: 8
#| fig-width: 10
...
df_muestra = df_muestra_final |> 
  select(codigo, censal, ivse_std, pi_corregido, nudge_power, factor_ajuste_CENS_EPA, expansor_establecimiento, ponderador_establecimiento, muestra_adultos, n_estudiantes_muestra, n_estudiantes_por_seccion)
